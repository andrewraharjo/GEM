...
static long int hex_bytes(double average);
...
int commander_init( int channel )
{
    ...
    int open_zero = OSCC_ERROR;

    if ( commander_enabled == COMMANDER_DISABLED )
    {
        commander_enabled = COMMANDER_ENABLED;

        return_code = epas_open( 0 );
        open_zero = oscc_open( channel );
...
static int command_steering( )
{
    ...
    if ( commander_enabled == COMMANDER_ENABLED && control_enabled == true )
    {
        ...
        if( return_code == OSCC_OK )
        {
            ...
            long steering_long = hex_bytes(average);
            unsigned char* steering_hex = (unsigned char *)&steering_long;

            printf("Steering bytes: %x%x%x%x \n", steering_hex[0], steering_hex[1], steering_hex[2], steering_hex[3]);

            return_code = oscc_publish_epos_value( steering_long );

            // return_code = oscc_publish_steering_torque( steering_hex );
        }
...
static long int hex_bytes(double average) {
    double steer_const = 1.0 / 620;
    long RATE = 1048576; 
    double steering_degrees = average / steer_const;
    double revolutions = steering_degrees / 360;
    long dec_rev = (long)( revolutions * RATE );
    return ( dec_rev );

}
// static int average_to_hex(long double average) {
//     int begin, end, count = 0;
//     long double steer_const = 1.0 / 620;
//     long int powered_const = 1048576;
//     float steering_degrees = average / steer_const;
//     float revolutions = steering_degrees / 360;
//     int decimal_revolutions = 0;
//     decimal_revolutions = revolutions * powered_const;
//     unsigned char hex[10];
//     unsigned char reversed_hex[10];
//     sprintf(hex, "%x", decimal_revolutions);
//     while (hex[count] != '\0')
//       count++;
...
//       reversed_hex[begin] = hex[end];
//       end--;
//     }

//     int hex_number;
//     sscanf( reversed_hex, "%x", & hex_number);
